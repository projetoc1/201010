...
void* f_thread1(void* data)
{
    ...
    while(1)
    {
        ...
        if(a==65)
        {
          ...
          for(ii = 1; ii <= 3; ii++)
          {
            c=serialGetchar(uart);
            valstring+=String(c-48);
            conv2=valstring.c_str();
          }
          //int beta = serialDataAvail(uart);
          //printf("valor = %d bytes %d \n",atoi(conv2),beta);
          printf("valor = %d\n",atoi(conv2));
            //cc=serialGetchar(uart);
            //printf("valor = %d %d bytes = %d  \n",c-48,cc-48,beta);
        }

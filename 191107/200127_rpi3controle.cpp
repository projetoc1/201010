...
int main()
{
    ...
    while(1)
    {
        ...
        if(modo_pm_pa == 1) //PA
        {
            ...
            //ajuste de altura e velocidade no joystick
            ...
            //altitude e theta
            //if que indica pa do simulink ou pa com h_ref variando no joystick
            ...
            if(flag_h==1)
            {
                //h_ref_expp = h_ref + (h - h_ref)*(1-exp(-(segundos-timehold_h_up)/10));
                //h_ref_expp = (h_ref-h)*(1-exp(-segundos/10));
                if(abs(h_ref-h)<2.5)
                {
                    h_ref_expp = h_ref;
                }
                ...
                {
                    h_ref_expp = h_inicial + 2.5*dt;
                }
                //h_ref_expp =h_ref + 2.5*dt;
                erro_h = h_ref_expp-h;
            }
        ...
        printf("a = %6.2f p = %6.2f l = %6.2f m = %6.2f h = %6.2f h_ref = %6.2f h_ref_expp = %6.2f v = %6.2f v_ref = %6.2f\n",comando_aileron,interconexao,comando_leme,v_pi,h,h_ref,h_ref_expp,v,v_ref);

...
float p_pitot_filtrada       = 0;
float p_pitot_filtrada_2     = 0;
float p_pitot_filtrada_ant   = 0;
float p_pitot_filtrada_ant_2 = 0;
float p_pitot_ant            = 0;
float v_aero_p_filtrado      = 0;
float v_aero_p_filtrado_2    = 0;
...
void* f_thread2(void* data)
{
    ...
    fprintf(texto,"              xI");
    fprintf(texto,"              zI");
    fprintf(texto,"        v_aero_f");
    fprintf(texto,"       v_aero_f2");
    fprintf(texto,"      v_aero_p_f");
    fprintf(texto,"     v_aero_p_f2");
    fprintf(texto,"\n");
    ...
    while(1)
    {
        ...
        else
        {
            ...
            fprintf(texto,"%16.2f",v_aero_p_filtrado);
            fprintf(texto,"%16.2f",v_aero_p_filtrado_2);
            fprintf(texto,"\n");
            usleep(20000); //50Hz
        }
    }
...
int main()
{
    ...
    while(1)
    {
        ...
        p_pitot_filtrada       = f_passa_baixa(p_pitot_filtrada_ant,dt,p_pitot_ant,10);
        p_pitot_filtrada_ant   = p_pitot_filtrada;
        p_pitot_filtrada_2     = f_exp(p_pitot_filtrada_ant_2, pressao_pitot, 0.01);
        p_pitot_filtrada_ant_2 = p_pitot_filtrada_2;
        p_pitot_ant            = pressao_pitot; 
        v_aero_p_filtrado      = sqrt((abs(p_pitot_filtrada)/(0.5*1.22501)));
        v_aero_p_filtrado_2    = sqrt((abs(p_pitot_filtrada_2)/(0.5*1.22501)));
        ...
        {
            contador=contador + 1;
            soma_aero = soma_aero + leitura_v_aero;
        }
        v_aero_filtrado = f_passa_baixa(v_aero_filtrado_ant,dt,v_aero_ant,1);
        v_aero_filtrado_ant = v_aero_filtrado;
        v_aero_filtrado_2 = f_exp(v_aero_filtrado_ant_2, leitura_v_aero, 0.01);
        v_aero_filtrado_ant_2 = v_aero_filtrado_2;
        v_aero_ant = v_aero_p_filtrado;

...
//TELA NEXTION
NexVariable flag_piloto = NexVariable(0,42,"flag_piloto");
...
int main()
{
    ...
    while(1)
    {
        ...
        if(botao_h_up == 1)
        {
            if(flag_botao_h_up_uma_vez == 0)
            {
                delta_h_ref = 50;
                h_ref = h_ref + delta_h_ref;
                flag_sinal_h = 1;
                time_hold_h = segundos;
                flag_botao_h_up_uma_vez = 1;
            }
        }
        else
        {
            flag_botao_h_up_uma_vez = 0;
        }
        if(botao_h_down == 1)
        { 
            if(flag_botao_h_down_uma_vez == 0)
            {
                delta_h_ref = -50;
                h_ref = h_ref + delta_h_ref;
                flag_sinal_h = -1;
                time_hold_h = segundos;
                flag_botao_h_down_uma_vez = 1;
            }
        }
        else
        {
            flag_botao_h_down_uma_vez = 0;
        }
        if(botao_v_up == 1)
        {
            if(flag_botao_v_up_uma_vez == 0)
            {
                delta_v_ref = 1.388888;
                v_ref = v_ref + delta_v_ref;
                time_hold_v = segundos;
                flag_botao_v_up_uma_vez = 1;
            }
        }
        else
        {
            flag_botao_v_up_uma_vez = 0;
        }
        if(botao_v_down == 1)
        {

            if(flag_botao_v_down_uma_vez == 0)
            {
                delta_v_ref = -1.388888;
                v_ref = v_ref + delta_v_ref;
                time_hold_v = segundos;
                flag_botao_v_down_uma_vez = 1;
            }
        }
        else
        {
            flag_botao_v_down_uma_vez = 0;
        }
        //fim do ajuste de setpoints
        ...
        printf("aileron = %6.2f profundor = %6.2f leme = %6.2f manete = %6.2f h = %6.2f h_ref = %6.2f h_ref_exp = %6.2f v = %6.2f v_ref = %6.2f flag_nextion %.0f\n",comando_aileron,interconexao,comando_leme,v_pi,h,h_ref,h_ref_exp,v*3.6,v_ref*3.6,flag_piloto_nextion);

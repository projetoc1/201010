...
int main()
{
    ...
    while(1)
    {
        ...
        if(modo_pm_pa == 1) //PA
        {
            ...
            float pa_ativo_ratio = 1000;
            //float pa_ativo_ratio = 1000000;

            // h_ref_exp = h*exp(-((segundos - tempo_pa_ativo)/pa_ativo_ratio)) + h_ref0*(1-(exp(-((segundos - tempo_pa_ativo)/pa_ativo_ratio))));

            // if(segundos - tempo_pa_ativo>120)
            // {
	        h_ref = h_ref_out;
            ...
            //ROTINA PID's
            float batente_v = 5;
            //PERTUBAÇÃO NA ENTRADA DE VELOCIDADE
            //perturbacao = manobras_seno(tempo_de_espera_pm_pa,segundos - time_hold_comandado,amplitude_de_comando,tempo_de_manobras,freq_vario);
            float pert_v  = manobras_seno(15,segundos - tempo_pa_ativo,0.5,30,0.5);
            float pert_v2 = (float)((random(30)-15)/10);
            
            float pert_v3;
            if(segundos - tempo_pa_ativo >15)
            {
            	fscanf(v_aero_table,"%f",&v_table);
            	pert_v3 = (v_table + 8)-27;
                v = v + pert_v3;
            }
            //tolerancia a variação de velocidade
            // float tolerancia_v = 2*1.388;
            // if(v >= v_ref_out + tolerancia_v || v <= v_ref_out - tolerancia_v)
            // {
            // 	v_filtrado = v;
            // }
            //FILTRO DE MEDIA
			// if(contador_aero>=5)
			// {
			// 	v_filtrado = soma_aero/5;
			// 	contador_aero = 1;
			// 	soma_aero = 0;
			// }
			// else
			// {
			// 	contador_aero = contador_aero + 1;
			// 	soma_aero = soma_aero + v;
			// }


            v_filtrado = f_passa_baixa(v_filtrado_antes,dt, v_ant,2.5);
            v_filtrado_antes = v_filtrado;
            // if(segundos - tempo_pa_ativo >15 && segundos - tempo_pa_ativo < 65)
            // {
            // 	fscanf(v_aero_table,"%f",&v_table);
            // 	v = v_table + 5;
            // }
            ...
            //MANOBRAS PRE-PROGRAMADAS////////////////////////////////////////////////////////////////////////////////////////////////////
        	float tempo_de_manobras = 2;
        	float tempo_de_espera_pm_pa = 12;
        	float amplitude_de_comando = 10;
        	float freq = 0.1;
        	float freq_max = 2;
        	...
        	{
        		flag_comandado = 1;
        		time_hold_comandado = segundos;
        	}
        	if(segundos - time_hold_comandado > tempo_de_espera_pm_pa)
        	{
        		freq_vario = (freq_max/tempo_de_manobras)*(segundos - time_hold_comandado);
        		freq_vario = freq_vario + contador_freq;
        		contador_freq = contador_freq + 0.00000001;
        		perturbacao = -1*(float)random(10)-5;
        		//comando_aileron = comando_aileron + perturbacao;
        	}
        	if(segundos - time_hold_comandado > tempo_de_espera_pm_pa + contador)
        	{
        		// freq_vario = (freq_max/tempo_de_manobras)*(segundos - time_hold_comandado);
        		// freq_vario = freq_vario + contador_freq;
        		contador = contador +2;
        		// contador_freq = contador_freq + 0.0001;
        		if(contador>100000)
        		{
        			contador = 0;
        			contador_freq = 0;
        	...
        	// if(segundos - time_hold_comandado > tempo_de_espera_pm_pa - 0.1 & time_hold_comandado <= tempo_de_espera_pm_pa)
        	// {
        	// 	comando_inicial = v_pi; 
        	// }
        	//perturbacao = manobras_seno(tempo_de_espera_pm_pa,segundos - time_hold_comandado,amplitude_de_comando,tempo_de_manobras,freq_vario);
        	//perturbacao = manobras(tempo_de_espera_pm_pa,segundos - time_hold_comandado,amplitude_de_comando,tempo_de_manobras);
       		//interconexao = interconexao + perturbacao;
        ...
        printf("tempo_pa_ativo =%8.2f v_ref =%8.2f v_filtrado =%8.2f v =%8.2f\n",segundos - tempo_pa_ativo,v_ref_exp,v_filtrado,v);
        usleep(20000); //50Hz
    }//while thread0
    return 0;
}

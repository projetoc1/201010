...
{
	float pi=atan(1)*4;
	if(segundos>40 && flag_ned0==0 && Horizontal_acc<=2 && Vertial_acc<=7) //(1)
	{
		lat0 = Latitude;
		lon0 = Longitude;
		height_e0 = height_e;
...
void* f_thread1(void* data)
{
    ...
    while(1)
    {
        ...
        int v_aero    = 3.6*sqrt((abs(pressao_pitot*6894.76)/(0.5*1.22501)));
        int alt       = height_msl;
        int rpm       = 0;
...
void* f_thread2(void* data)
{
    ...
    while(1)
    {
        ...
        else
        {
            ...
            fprintf(texto,"%16.2f%16.2f%16.2f",Horizontal_acc,Vertial_acc,mzlsm); //gps - precisão horizontal 2d e 3d

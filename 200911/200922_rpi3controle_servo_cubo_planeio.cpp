...
float h_ref_novo      = 0;
int   flag_planeio    = 0;
float tinic_planeio   = 0;
float h_ref_planeio   = 0;
float flag_h_plato    = 0;
...
float Vini; // valor de velocidade no instante a partir do qual a trajetoria otima sera utilizada
float v_delta_inicial = 0; //delta de velocidade entre a velocidade da aeronave e o primeiro elemento de traj_V
...
void v_shift(void) // subtrai a velocidade inicial "Vini" em todos os elementos de "traj_V", para começar em zero
{
  int i = 0;
  while (i < n_lines_util)
  {
    traj_V[i] = traj_V[i] - Vini;
    i = i + 1;
  }
}
...
void interpol(float tempo, float array[])
{
    ...
    else
    {
        ...
        array[1] = v_out;
        array[0] = h_out;
        // printf("t1 = %6.2f t2 = %6.2f tempo = %6.2f v1 = %6.2f v2 = %6.2f v_out = %6.2f h1 = %6.2f h2 = %6.2f h_out = %10.6f arr0 = %6.2f arr1 = %6.2f\n",t1,t2,tempo,v1,v2,v_out,h1,h2,h_out,arr[0],arr[1]);
        
    }
    //printf("h_ref =%6.2f h_traj =%6.2f v_ref =%6.2f v_traj =%6.2f\n",h_plato, arr[0],v_ref,arr[1]);
    // printf("t1 = %6.2f t2 = %6.2f tempo = %6.2f tmax = %6.2f v1 = %6.2f v2 = %6.2f v_out = %6.2f h1 = %6.2f h2 = %6.2f h_out = %6.2f arr0 = %6.2f arr1 = %6.2f E = %6.4f\n",t1,t2,tempo,t_max,v1,v2,v_out,h1,h2,h_out,arr[0],arr[1],energia);
}
      ...
      fprintf(texto,"         energia");
    	...
    	while(1)
    	{
    		...
    		else
    		{
                ...
                fprintf(texto,"%16d",iotim);
                fprintf(texto,"%16.6f",energia);
    			fprintf(texto,"\n");
...
int main()
{
    ...
    while(1)
    {
        ...
        if(modo_pm_pa == 1) //PA
        {
            ...
            if(segundos - tempo_pa_ativo >= 1 && flag_atualiza_trajetoria == 0)
            {
              flag_atualiza_trajetoria = 1;
              tempo_atualiza_trajetoria = segundos;
            }

            if(segundos - tempo_atualiza_trajetoria >= 20 && flag_trajetoria == 0)
            {
              h_ref_traj = h;
              flag_trajetoria == 1;
              tempo_atualiza_trajetoria = segundos;
              ...
              printf("E = %4.2f h_i = %7.2f h_f = %7.2f\n",energia,h_i,h_f);
              ...
              printf("E = %4.2f h_i = %7.2f h_f = %7.2f\n",energia,h_i,h_f);
              printf("iE = %d ih_i = %d ih_f = %d trajetoria : %d\n",iE,ihi,ihf,iotim);
              ...
              // Vini = traj_V[0]; // valor de velocidade no instante a partir do qual a trajetoria otima sera utilizada
              t_shift(); // subtrai o instante inicial "tini" em todos os elementos de "traj_t", para começar em zero
              h_shift();
              // v_shift();

              //DESCIDA FIXA P/ SITUAÇÕES FORA DO CUBO
              if (iinterc == -1)
              {
                flag_planeio = 1;
                tinic_planeio = segundos;
              }
              else
              {
                flag_h_plato = 0;
            ...
            if(segundos - tempo_atualiza_trajetoria >= 0)
            {
              flag_trajetoria = 0;
              tempo_atual = segundos - tempo_atualiza_trajetoria;
              interpol(tempo_atual, arr);
              // printf("arr0 %8.4f arr1 %8.4f\n",arr[0],arr[1]);
              v_ref_exp_tr = (arr[1] - v_delta_inicial) + (v_delta_inicial)*(1 - exp(-(segundos - tempo_atualiza_trajetoria)/(abs(v_delta_inicial)/v_ratio)));
            ...
            //trajetoria cubo
            if(segundos - tempo_pa_ativo < 1)
            {
              h_ref_traj = h_ref;
            }

            h_plato = h_ref_traj + arr[0];
            //h_plato = h_ref + arr[0];
            
            if(flag_planeio == 1 && flag_h_plato == 0)
            {
              h_ref_planeio = h;
              flag_h_plato = 1;
            }
            ...
            //PLANEIO FIXO
            float razao_descida = -1.25; //  m/s
            if(flag_planeio == 1 && tinic_planeio > 0)
            {
              h_plato = h_ref_planeio + (segundos - tinic_planeio)*razao_descida;
            }
            ...
            //trajetoria cubo
            float v_descida_fixa = 27.78;   //   m/s
            if(segundos - tempo_pa_ativo >= 20)
            {
               // v_ref = v_delta_inicial + v_ref_out + arr[1];
              // v_ref = v_ref_out + arr[1];
               // v_ref = arr[1];
               v_ref_exp = v_ref_exp_tr;

               //PLANEIO FIXO
               if(flag_planeio == 1)
               {
                 v_ref_exp = v_descida_fixa;
               }



               // printf("v_ref: %8.4f v: %8.4f delta_v: %8.4f\n",v_ref,v,v_ref - v);
            }
            else
            {
              v_ref_exp = (v_ref - delta_v_ref) + (delta_v_ref)*(1 - exp(-(segundos - time_hold_v)/(abs(delta_v_ref)/v_ratio)));
            }
            printf("h_plato: %7.2f v_ref_exp: %7.2f\n",h_plato,v_ref_exp);
            //v_ref_exp = (v_ref - delta_v_ref) + (delta_v_ref)*(1 - exp(-(segundos - time_hold_v)/(abs(delta_v_ref)/v_ratio)));

...
void* f_thread1(void* data)
{
    ...
    while(1)
    {
      ...
      // 			j_traj ++;
        ...
        // tempo_atual = segundos - tempo_pa_ativo;
      //   if(modo_pm_pa == 1 && segundos - tempo_atualiza_trajetoria > 20)
      //   {
      ...
      //   printf("h: %7.2f h_ref: %7.2f trajetoria : %d\n",h,h_plato,iotim);
    }
    close( js );
}
...
int main()
{
    ...
    while(1)
    {
        ...
        if(modo_pm_pa == 1) //PA
        {
            ...
            if(segundos - tempo_pa_ativo > 0.001)
            {
               // tempo_atual = segundos - tempo_pa_ativo - 1;
              if(modo_pm_pa == 1 && segundos - tempo_atualiza_trajetoria > 20)
              {
                flag_atualiza_trajetoria = 1;
                tempo_atualiza_trajetoria = segundos;
            ...
            // if(tempo_atualiza_trajetoria<5)
            // {
            //   h_plato = (h_ref + arr[0] - h) + (h_ref + arr[0] - h)*(1 - exp(-(segundos - time_hold_h)/(abs(h_ref + arr[0] - h)/h_ratio)));
            // }
            // else
            // {
            //   h_plato = h_ref + arr[0];

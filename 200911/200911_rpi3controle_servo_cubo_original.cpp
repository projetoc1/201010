...
void interpol(float tempo, float array[])
{
    ...
    float t_max = 0;
     ...
     //          while(j_traj<400)
     //          {
     //            printf("t = %6.2f h = %6.2f v = %6.2f indice : %d\n",traj_t[j_traj],traj_h[j_traj],traj_V[j_traj],j_traj);
     //            j_traj ++;
     //          }
     //          printf("trajetoria : %d\n",iotim);
...
int main()
{
    ...
    while(1)
    {
        ...
        if(modo_pm_pa == 1) //PA
        {
            ...
            {
              //printf("h: %7.2f h_ref: %7.2f trajetoria : %d h_f %7.2f\n",h,h_plato,iotim,h_f);
              h_ref_traj = h;
              flag_trajetoria == 1;
              tempo_atualiza_trajetoria = segundos;
              tempo_atual = 0;
            ...
            if(segundos - tempo_pa_ativo >= 20)
            {
              ...
              // v_ref = v_ref_out + arr[1];
               v_ref = arr[1];
               // printf("velocidade :%7.4f vel interpolado :%7.4f vel traj :%7.4f trajetoria %d\n",v,v_ref,arr[1]+Vini, iotim);
            }
